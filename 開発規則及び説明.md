...
- Urlの共用部分の定数を格納されているクラス  
  [UrlConst.java](src%2Fmain%2Fjava%2Fcom%2Fpots%2Fcommon%2Fconstant%2FUrlConst.java)
    ```java
    private static final String UPLOAD_FOLDER = "http://localhost:7032/uploads/";
    ```
...
- 下記[Result](#Result)クラス静的メソッドResult.successや.failedメソッド内部で使用されるため、呼び出し元は意識せず統一的にレスポンスが作られる
